...
impl<B: Backend> MyModel<B> {
    fn new(device: &B::Device, d_in: usize, d_hidden: usize, d_out: usize) -> Self {
        Self {
            ...
            layer2: nn::LinearConfig::new(d_hidden, d_out).init(device),
        }
    }
    ...
    fn forward(&self, x: Tensor<B, 2>) -> Tensor<B, 2> {
        ...
        let x = DropoutConfig::new(0.2).init().forward(x);
    ...
    fn load(path: &str, device: &B::Device, d_in: usize, d_hidden: usize, d_out: usize) -> Result<Self, Box<dyn Error>> {
        let recorder = CompactRecorder::new();
        let record = recorder
...
fn load_data_extended() -> Result<(Vec<f32>, Vec<i64>, usize), Box<dyn Error>> {
    ...
    let valid_mask = joined
        .column("rating")?.is_not_null()
    ...
    println!("Features étendues: {} genres + 3 autres = {} features totales",
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let n_samples = labels.len().min(max_samples);
    */
    ...
    let train_features: Vec<f32> = train_idx.iter()
        .flat_map(|&i| {
    ...
    let test_features: Vec<f32> = test_idx.iter()
        .flat_map(|&i| {
            let start = i * nb_features;
            let end = start + nb_features;
            features[start..end].to_vec()
    ...
    let y_test = Tensor::<MyBackend, 1, Int>::from_ints(&*test_labels, &device)
        .reshape([test_labels.len()]);

    println!("Dataset étendu: {} échantillons avec {} features", n_samples, nb_features);
    println!("Train: {} échantillons, Test: {} échantillons", train_labels.len(), test_labels.len());
    ...
    let optimizer_config = SgdConfig::new().with_gradient_clipping(Option::from(GradientClippingConfig::Norm(1.0)));
    ...
    for epoch in 0..1000 {
        for batch_start in (0..train_labels.len()).step_by(batch_size) {
            ...
            let x_batch = x_train.clone().slice([batch_start..batch_end, 0..nb_features]);
        ...
        if epoch % 10 == 0 {
            ...
            let acc_test = preds_vec.iter().zip(test_labels.iter())
                .filter(|(p, l)| p == l)
                .count() as f32 / test_labels.len() as f32;

            println!("Epoch {}: perte train = {:.4}, précision test = {:.2}%",
                     epoch, loss_train.into_scalar(), acc_test * 100.0);
            ...
            if patience_counter >= patience {
                println!("Early stopping à l'epoch {} (précision test ne s'améliore plus)", epoch);
                break;
            }
        }
    }
    ...
    let acc = preds_vec.iter().zip(test_labels.iter())
        .filter(|(p, l)| p == l)
        .count() as f32 / test_labels.len() as f32;
